...
<script>
  ...
  import AssetsPage from '@/views/project/task/components/assets-page-studentList'
  import { queryStudent } from 'api/studentAPI'
  import { removeStudentsFromClass } from 'api/classAPI'
  ...
  export default {
    ...
    methods: {
      ...
      async deletePageData () {
        console.log('asdada')
        const res = await removeStudentsFromClass(this.multipleSelection, this.$route.params.classId)
        if (res.success) {
